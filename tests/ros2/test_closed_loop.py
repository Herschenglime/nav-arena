...
def test_closed_loop():
    ...
    received_goal_reached: list[Bool] = []
    ...
    test_node.create_subscription(PoseStamped, "/goal_pose", lambda m: received_goals.append(m), latched_qos)
    test_node.create_subscription(Bool, "/goal_reached", lambda m: received_goal_reached.append(m), latched_qos)
    ...
    try:
        ...
        # Robot spawns at map x=-2.5, goal is at map x=-1.0 (delta = 1.50m)
        target_goal_dist = abs(goal.pose.position.x - (-2.5))
        max_drive_timeout = 60.0 # 60 seconds allows full navigation even on shader compiling kit launch

        print(f"[INFO] Goal Navigation Target Distance: {target_goal_dist:.2f}m ahead")
        ...
        while time.time() - drive_start < max_drive_timeout and sim_proc.poll() is None:
            ...
            max_traveled = max(max_traveled, curr_traveled)
            ...
            # Check goal reached success condition via decoupled /goal_reached topic
            if len(received_goal_reached) > 0 and received_goal_reached[-1].data:
                print("=" * 70)
                print(f"[SUCCESS] GOAL REACHED! Received /goal_reached=True (max traveled: {max_traveled:.3f}m)")
                print("=" * 70)
                goal_reached = True
        ...
        assert goal_reached, f"Robot failed to reach goal within {max_drive_timeout}s! Max traveled: {max_traveled:.3f}m"
        ...
        print(f"       Total /goal_reached messages received: {len(received_goal_reached)}")
        ...
        assert len(received_goal_reached) > 0 and received_goal_reached[-1].data is True, "No /goal_reached True received."
        assert len(received_odom) > 10, "Insufficient /odom messages received."

        # Verify goal pose coordinates match expected (-1.0, 0.0) in map frame
        final_goal = received_goals[-1]
        assert final_goal.header.frame_id == "map", f"Unexpected frame_id: {final_goal.header.frame_id}"
        ...
        print(f"[INFO] Odometry verification: max forward distance navigated = {max_traveled:.3f}m")
        assert max_traveled >= 1.0, f"Robot traveled insufficient distance: {max_traveled:.3f}m"
